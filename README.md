...
Every conversation you have in LM Studio is stored as a `.conversation.json` file. As your chats grow, these files accumulate metadata and older turns that keep getting resent to the model on every message — quietly burning through context window space. This plugin steps between messages (via the prompt preprocessor hook) to clean those files up behind the scenes.


On each trigger it performs several passes of housekeeping:


- **Truncation** – Keeps a configurable number of the oldest and newest user and assistant messages, discarding everything in between that is no longer needed for an ongoing response. If you wish not to lose an assistant response out of context, you have three options: widen the range of kept messages, override cleanup but keep all messages, or use my [GitHub - Persisting Memories Explicit](https://github.com/anh-vudinh/-anh-vudinh-LM-Studio_Plugin-Persisting-Memories-Explicit) to save an assistant's response into a memory seed. You can then inject that back into the conversation for the assistant to reference. You will not visually see the memory seed injected into chat unless you specifically ask the assistant to read the memory back out to you, which would technically waste tokens by duplicating information already present in context.
- **Thinking cleanup** – Removes previously generated thinking steps from assistant messages so reasoning tokens aren't re-sent forever. The last assistant thinking is excluded and preserved.
...
- **Metadata preservation** – Carries over critical information such as the Internal Chat ID (`ICID`), any memory seeds from the companion plugin [GitHub - Persisting Memories Explicit](https://github.com/anh-vudinh/-anh-vudinh-LM-Studio_Plugin-Persisting-Memories-Explicit), and Formatting Instructions — including the message number append from the Model Dependent version of PM. This works correctly across cleanup cycles.
...
Make sure **`Cleanup Context?`** is enabled alongside enabling the plugin — cleanup won't run at all if this is off.
...
4. **Let the plugin do its work automatically.** A mandatory 2-second grace period is required immediately after the assistant finishes its response before cleanup can occur. Once trigger conditions are met, it cleans up and maintains the current conversation file behind the scenes.
5. **(Optional) Enable backup mode** to keep an uncleaned copy of your conversations for reference or recovery. If you enable this mid-conversation, the *first moment* you turn it on becomes the foundation of the backup's data. Leave it toggled on if you want the model to continue updating that respective backup file. The moment you turn it off, the newest message turn stops being appended.


## How It Works


![Image of Plugin Control Panel](control-panel.jpg)
...
| **OVERRIDE: Cleanup But Keep All Messages** | Boolean · `ON / OFF` | When enabled, internal cleaning still runs but no turns are truncated — everything is retained in context. This overrides the keep-oldest / keep-newest counts system. |
| **OVERRIDE: Cleanup Thinking Context Only** | Boolean · `ON / OFF` | Restricts cleanup to thinking tokens only; all other default cleanups (system prompt, Jinja templates, tools, preprocessed content) are ignored. |
| **OVERRIDE: Keep All Thinking Context** | Boolean · `ON / OFF` | Disables thinking cleanup. All thinking will remain. Other cleanups still apply. |
...
| :--- | :--- |
| **USER MESSAGE 1** | The first user's message is used to house metadata from my [GitHub - Persisting Memories Explicit](https://github.com/anh-vudinh/-anh-vudinh-LM-Studio_Plugin-Persisting-Memories-Explicit) plugin. This includes the ICID, memories the user injects, and formatting instructions. You can freely delete the user's message 1 — there is built-in recovery so nothing will break. This is just for your information; nothing is required of you on this front. |
...
| `<conversationFile>.lock` *(temporarily inside `/conversations`)* | A temporary lock file created while cleanup is in progress and removed when finished. This lets this plugin safely run alongside others, such as my [Persisting Memories](https://github.com/anh-vudinh/LM-Studio_Plugin-Persisting-Memories) plugin, that may want to modify the conversation at the same time. |
...
- This plugin is also recommended to run with my Persisting Memories plugin because it can help clean up some artifacts used to run that plugin, like renumbering message #s and relocating some metadata to the first user's message so there are fewer resources used while iterating through the conversation file.
- The plugin will not be able to delete the user's first message, but that does not prevent you from deleting it using LM Studio's standard "Delete this message" button tied to each chat bubble. I've gone ahead and added the ability for the plugin to repopulate that metadata on its own in the user's new "first message." I've covered many failure points with recovery built in — just do what you wish to do how you wish to do it. The only thing you have to respect is the 2-second time window after the assistant's finished response. That constraint is not on me; that's a limitation of LM Studio, and I can only ensure nothing breaks depending on what you do.